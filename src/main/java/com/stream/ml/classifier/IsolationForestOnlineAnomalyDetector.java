...
public class IsolationForestOnlineAnomalyDetector implements Serializable {
	
	private final static Logger logger = LoggerFactory.getLogger(IsolationForestOnlineAnomalyDetector.class.getName());
    ...
    public IsolationForestOnlineAnomalyDetector(List<String[]> attributes) throws FileNotFoundException {
    ...
    public void onlineFit(AccessEvent[] accessEvents) {
		...
		if (null == isolationForest) {
			synchronized (IsolationForestOnlineAnomalyDetector.class) {
    ...
    public boolean isAnomaly(AccessEvent accessEvent) {
    	...
    	logger.info("evalulate: "+String.format("%,.4f", isolationForest.evaluate(row))+", event: "+accessEvent+", tensor : "+(row==null? "" : Arrays.toString(row.toArray())) );
   ...
   public static void main(String[] args) throws IOException, FileNotFoundException {
       ...
       IsolationForestOnlineAnomalyDetector algorithm = new IsolationForestOnlineAnomalyDetector( attributes);
